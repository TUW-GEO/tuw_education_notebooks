...
def view_seasonal_ssm(quarterly_datacube, cmap):
    ...
    for i, j in enumerate([m, m2, m3, m4]):
        ...
        # j.add_feature.preset.land(hatch="////")
        j.set_data(quarterly_datacube.isel(time=i), x="x", y="y", parameter="band_data", crs=Maps.CRS.Equi7_EU)
        j.plot_map(cmap=cmap, zorder=3)

    m.add_colorbar(label="surface soil moisture (1)", pos=(0.1, 0.1, 0.8, 0.1))

    m.apply_layout(
        {
            "figsize": [6.4, 4.8],
            "0_map": [0.1, 0.6, 0.5, 0.5],
            "1_map": [0.6, 0.6, 0.5, 0.5],
            "2_map": [0.1, 0.1, 0.5, 0.5],
            "3_map": [0.6, 0.1, 0.5, 0.5],
        }
    )
    m.show()